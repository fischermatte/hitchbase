...
class gmap {
     ...
     * DEPRECATED:
     * API Key sollte jetzt im Konstruktor der Klasse �bergeben werden.
     * Beispiel:
    ...
    function headjs($key = "")
    {
        ...
        echo "<script src=\"http://maps.google.com/maps?file=api&amp;v=2&amp;key=$key\" type=\"text/javascript\" language=\"javascript\"></script>\n
			  <style type=\"text/css\">\n
	    	  v\:* {\n
     ...
     * @param int $height H�he der Karte
     * @return bool
     */
     ...
     * @param int $lat Initialer Breitengrad
     * @param int $lng Initialer L�ngengrad
     * @param string $typ Kartentyp (normal,satellit,hybrid) Default: normal
     * @param int $min_zoom Minimal m�gliche Zoomstufe
     * @param int $max_zoom Maximal M�gliche Zoomstufe
     * @param string $navcontrol Typ der Kartennavigation (small,large) Default: large
     * @param int $overview Schaltet die �bersichtskarte an oder aus.
    ...
    {
        echo "
		<script type=\"text/javascript\" src = \"bodyjs.js\"></script>\n";
        return true;
    }

    /**
     * Erzeugt ben�tigten JS Code im Seitenfuss
     *
     * @return bool
     */
    function footjs()
     ...
     *
     * @param int $lat Breitengrad des Markers
     * @param int $lng L�ngengrad des Markers
     * @param string $img Steuerung der Grafik
     * @return bool
     */
    function marker($lat, $lng, $img = "143.png")
    {
        if ($lat and $lng) {
     ...
     * @param int $lat Breitengrad des Markers
     * @param int $lng L�ngengrad des Markers
     * @param string $html HTML Code f�r das Infopopup
     * @param string $img Steuerung der Grafik
     * @return bool
     */
    function otherMarker($lat, $lng, $html = "", $img = "143.png")
    {
        $dims = getimagesize($img);
        if ($lat and $lng) {
            echo "var image = new Array(\"$img\",$dims[0],$dims[1]);\n";
     ...
     * @return bool
     */
    function tabMarker($lat, $lng, $tabs, $img = "143.png")
    {
        if ($lat and $lng) {
            $dims = getimagesize($img);
            echo "var image = new Array(\"$img\",$dims[0],$dims[1]);\n";
     ...
     * @return bool
     */
    function linkMarker($lat, $lng, $link, $img = "143.png")
    {
        if ($lat and $lng) {
            $dims = getimagesize($img);
            echo "var image = new Array(\"$img\",$dims[0],$dims[1]);\n";
    ...
    /**
     * Erzeugt einen Marker dessen Infopopup eine Detailkarte enth�lt
     *
     * @param int $lat Breitengrad des Markers
     * @param int $lng L�ngengrad des Markers
     * @param string $img Steuerung der Grafik
    ...
    /**
     * Erzeugt ein Geocoding Request und gibt eine Polyline, sowie Ortsliste zur�ck
     *
     ...
     * @return bool
     */
    function directions($start, $end, $startmarker = "143.png", $endmarker = "143.png", $zoombounds = 0, $textbox = "")
    {
		...
		var marker = createMapMarker(step_last, image);
		map.addOverlay(marker);";
        if ($textbox != "") {
    ...
    /**
     * Erweitert ein vorhandenes Bounds Objekt um den zuletzt hinzugef�gten Punkt.
     *
     * @return bool
     */
     ...
     * @param string $arrows Richtungspfeile. "all" = �berall, mid = "nur auf der Wegstrecke" , "end = "nur am "Ende"
     * @param int $geodetic Legt fest ob "geod�sische" Polylines gezeichnet werden. Diese ber�cksichtigen die Erdkr�mmung.
     * @return bool
     */
    function polyline($punkte, $color = "#FF0000", $breite = 1, $transparent = 0, $arrows = "", $geodetic = 0)
    {
        $polystr = "";
        $pts = "[";
    ...
    /**
     * Ermittelt die L�nge einer Polyline in Metern.
     *
     ...
     * @return array
     */
    function geocode($adress, $typ = 'marker', $img = "143.png", $zoom = 14)
    {
        if ($adress) {
            $dims = getimagesize($img);
            echo "var image = new Array('$img',$dims[0],$dims[1]);\n";
    ...
    /**
     * F�hrt ein Geocoding Request mit dem Google Geocoder aus.
     * Die Anfrage sollte so aufgebaut sein: <strasse> <plz> <stadt> <land>
     *
     * @param string $input Geocoding Anfrage
     * @param string $key Google Maps API Key (nur f�r R�ckw�rtskompatibilit�t)
     * @return array $point
     */
    ...
    /**
     * Parsed XML Dateien und gibt Geopunkte als Array zur�ck
     *
    ...
    /**
     * Parsed XML Dateien und gibt Marker als Array zur�ck.
     *
     * @param string $file Zu parsende Datei
     * @return array
     */
